...
class add_reaction implements add_interface
{
	...
	public function add($post_id, $type_id)
	{
		...
		$sql = 'SELECT post_id, poster_id, post_subject, topic_id, post_reaction_data, post_reactions
			FROM ' . POSTS_TABLE . '
		...
		$delete_url = $this->auth->acl_get('u_delete_reactions') ? $this->helper->route('steve_postreactions_delete_reaction_controller', ['post_id' => $post_id, 'user_id' => $this->user->data['user_id'], 'hash' => generate_link_hash('delete_reaction')]) : '';
		$view_url = $this->auth->acl_get('u_view_post_reactions_page') && !empty($this->config['reactions_posts_page_enabled']) ? $this->helper->route('steve_postreactions_view_reactions_controller_pages', ['post_id' => $post_id]) : '';
		...
		{
			//try ($this->type_operator->ajust_type_add($type_id, $new_type_counted, false)
